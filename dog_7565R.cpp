...
Vars: CS-Pin, MOSI-Pin, SCK-Pin (MOSI=SCK Hardware else Software), A0-Pin (GPIO_PIN_SET=data, GPIO_PIN_RESET=command), type (1=EA DOGM128-6, 2=EA DOGL128-6)
------------------------------*/
void dog_7565R::initialize(spi_api *spi, gpio_api *port_a0, int pin_a0, byte type)
{
	byte *ptr_init;	  //pointer to the correct init values
	top_view = false; //default = bottom view

	dog_7565R::port_a0 = port_a0;
	dog_7565R::pin_a0 = pin_a0;
	dog_7565R::spi = spi;

	port_a0->pinMode(pin_a0, OUTPUT);
	...
	port_a0->digitalWrite(pin_a0, LOW); //init display
...
void dog_7565R::clear(void)
{
	...
	{
		position(0, page);
		port_a0->digitalWrite(pin_a0, HIGH);
		spi->start();
...
{
	port_a0->digitalWrite(pin_a0, LOW);
	spi_put_byte(dat);
}
